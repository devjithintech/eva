...
function basisOf(shareClass: string): string {
...
export function RiskSection({ rec }: Props) {
  ...
  const rows = snapshots.map((entry) => {
    ...
    return { entry, fundRef, isSubject, isSibling, basis: basisOf(str(entry.share_class)) };
  });
  ...
  return (
    <section id="risk" className="sec">
      <div className="sec-head">
        ...
        {rows.length > 0 && (
          <span className="seg">
            ...
            <button type="button" className={view === "card" ? "on" : ""} onClick={() => setView("card")}>
              Card
            </button>
          </span>
        )}
      ...
      <div className="sec-body">
        {rows.length === 0 && <p className="note">No downside/risk data on record for this candidate.</p>}
            ...
            <table className="data risk-tbl">
              ...
              <tbody>
                {rows.map((r, i) => (
                  <tr key={i}>
                    <td>
                      <div className="ent-name">{r.isSubject ? "Subject fund" : r.isSibling ? "Sibling" : "Other"}</div>
                      <div className="ent-sub">{r.fundRef}</div>
                    </td>
                    <td>{r.basis !== "—" ? <span className={`bpill ${r.basis}`}>{r.basis}</span> : "—"}</td>
                    <td className="per" title={str(r.entry.period)}>
                      {str(r.entry.period)}
                    </td>
                    <td className="num">{pct(r.entry.volatility_pct)}</td>
                    <td className="num dd">{pct(r.entry.max_drawdown_pct)}</td>
            ...
            {rows.map((r, i) => (
              <div className="card" key={i}>
                <div className="card-head">
                  ...
                  {str(r.entry.period) !== "—" && <span className="meta">{str(r.entry.period)}</span>}
                </div>
                {str(r.entry.note) !== "—" && <p className="note">{str(r.entry.note)}</p>}
                <div className="grid">
