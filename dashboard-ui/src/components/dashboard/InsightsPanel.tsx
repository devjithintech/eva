...
import { AnalyzedInsights } from "./insights/AnalyzedInsights";
import { ShortlistedInsights } from "./insights/ShortlistedInsights";
import { InterviewInsights } from "./insights/InterviewInsights";

interface Props {
  tab: DashboardTab;
  /** Saved Alpha range from the Preferences dialog — drives the Analyzed
   *  tab's Selection Zone chart (which candidates qualify + the drawn zone). */
...
export function InsightsPanel({ tab, alphaRange }: Props) {
  ...
  return (
    <div className={`insights${collapsed ? " collapsed" : ""}`}>
      ...
      {!collapsed && (
        <>
          {tab === "analyzed" && <AnalyzedInsights alphaRange={alphaRange} />}
